...
public class InvestigateLView extends Layer.LView{
	...
	InvestigateLView(InvestigateLayer layer, boolean isMainView){
		super(layer);
		iLayer = layer;
		if(isMainView){		
	...
	protected LView _new() {
		return new InvestigateLView((InvestigateLayer)getLayer(), false);
	}
	...
	public synchronized void paintComponent(Graphics g){
	    // If we're exporting, don't draw the investigate tool's data.  We'll grab it in at the end and scale it up
		...
		for(DataSpike ds : iLayer.getDataSpikes()){
			...
			if(ds.isLabelOn()){
				...
				Font labelFont = new Font("Arial", Font.BOLD, ds.getLabelSize());

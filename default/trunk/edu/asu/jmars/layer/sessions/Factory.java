...
public class Factory extends LViewFactory {
	...
	private static boolean isNetworkAvailable() {
		...
		try {
		    String url = "http://appserver.mars.asu.edu";
...
//			URL url = new URL("http://jmars.mars.asu.edu");
//			URLConnection conn = url.openConnection();
//			conn.connect();
//			connectionConfirmed = true;
//      } catch (UnknownHostException e) {
		} catch (URISyntaxException uhe) {
			System.err.println("JMARS network is unavailable. Some layers may not be available.");
			log.println("JMARS network is unavailable - UnknownHostException. Some layers may not be available.");
		} catch (Exception e) {
			System.err.println("JMARS network is unavailable. Some layers may not be available.");
			log.println("JMARS network is unavailable - Other Exception. Some layers may not be available.");
		}
...
//			int code = client.executeMethod(get);                                   // TODO (PW) replace
            ...
            if (!status) {
				System.err.println("HTTP code " + request.getStatus() + " received when downloading session from " + url);
				return;
			}

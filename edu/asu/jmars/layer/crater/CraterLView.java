...
public class CraterLView extends LView {
	...
	protected ArrayList<Crater> selectedCraters = new ArrayList<Crater>();
	...
	ArrayList<Crater> matchingCraters = new ArrayList<Crater>();
	...
	public CraterLView(final CraterLayer layer){
		super(layer);
		...
		addMouseListener(new MouseListener(){		
			public void mouseReleased(MouseEvent e) {
				if(isRubberband && mode == SELECT_MODE) // second conditional not really needed, but JIC
				{
					...
					for (Crater c : clayer.settings.craters)
					{
					...
					if(points.size() == 2){
						...
						if(!clayer.settings.filterMainView || meetsFilter){
							clayer.settings.craters.add(newCrater);
							((CraterTableModel)fp.table.getUnsortedTableModel()).addRow(newCrater);
							drawCraters();
							repaint();
						}
				...
				if (ToolManager.getToolMode() == ToolManager.SEL_HAND){
					if (e.getButton()==MouseEvent.BUTTON1) {
						if (mode == ADD_MODE) {
							...
							if (clayer.settings.toggleDefaultCraterSizeReset) {
								newCrater.setDiameter(clayer.settings.getNextSize());
								clayer.settings.setNextSize(clayer.settings.getDefaultSize());
								((CraterFocusPanel)getFocusPanel()).newCraterSize.setText(""+clayer.settings.getNextSize());
								repaint();
							} else {
								newCrater.setDiameter(clayer.settings.getNextSize());
							};
						
							newCrater.setColor(clayer.settings.nextColor);
							newCrater.setComment(clayer.settings.colorToNotesMap.get(clayer.settings.nextColor));
							
							CraterFocusPanel fp = (CraterFocusPanel)getFocusPanel();
	
							boolean meetsFilter=fp.meetsFilter(360-spatial.getX(), spatial.getY(), clayer.settings.getNextSize());
							...
							if (clayer.settings.filterMainView && !meetsPointFilter) {
							...
							if (points.size() == 3) {
								try {
									...
									crater.setColor(clayer.settings.nextColor);
									crater.setComment(clayer.settings.colorToNotesMap.get(clayer.settings.nextColor));
									boolean meetsFilter=fp.meetsFilter(360.0-spatialCenter.getX(), spatialCenter.getY(), crater.getDiameter());
	
									if (!clayer.settings.filterMainView || meetsFilter) {
										clayer.settings.craters.add(crater);
									
										((CraterTableModel)fp.table.getUnsortedTableModel()).addRow(crater);
			...
			public void mouseClicked(MouseEvent e) {
				...
				{
					if (clayer.settings.craters != null) {
						Crater crater = findCraterByScreenPt(e.getPoint());
...
//	 */
	...
	private Crater findCraterByWorldPt(Point2D worldPt)
	{
		...
		for (Crater crater : clayer.settings.craters) {
...
//		g2=viewman.wrapScreenGraphics(g2);
	...
	protected void deleteSelectedCraters() {
		clayer.settings.craters.removeAll(selectedCraters);
		selectedCraters.clear();
	...
	protected void deleteLastCrater() {
		if (clayer.settings.craters.size() > 0) {
			Crater crater = clayer.settings.craters.get(clayer.settings.craters.size() - 1);
			if (clayer.settings.craters.remove(crater)) {
				CraterTable table = ((CraterFocusPanel)focusPanel).table;
				table.getTableModel().removeAll();
				for (Crater c: clayer.settings.craters) {
					table.getTableModel().addRow(c);
	...
	private Crater getCurrentCircle() {
		...
		if(points.size() >= 2){
			crater = new Crater();
			crater.setUser(Main.USER);
			Color color = clayer.settings.nextColor;
			crater.setColor(color);
			crater.setComment(clayer.settings.colorToNotesMap.get(color));

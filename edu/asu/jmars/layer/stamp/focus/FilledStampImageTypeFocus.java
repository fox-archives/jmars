...
public class FilledStampImageTypeFocus extends FilledStampFocus{
	...
	static {
		...
		for (int i=0; i<panSizeList.length; i++)
			try {
				URL url = Main.getResource("resources/pan_" + panSizeList[i] + ".gif");
	...
	private class PanButton extends JButton {
		...
		// Movement button
		private PanButton(final int x, final int y) {
			...
			Icon dirIcon = null;
			try{
				dirIcon = new ImageIcon(Main.getResource("resources/pan_" + dir + ".gif"));
			}
			catch(Throwable e){

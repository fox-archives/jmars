...
//			System.out.println(pt.getX()+", "+pt.getY());
	...
	//recalculates the path shape to be used for calculating intersection with the selection box, based off current projection
	public GeneralPath calcWorldPath(MultiProjection mp){
		ArrayList<Point2D> pts = new ArrayList<Point2D>(points.size());
		for(Point2D pt: points){
			pt = mp.spatial.toWorld(pt);
			if(pt.getX()>180){

...
public class DownloadAndParseNomenclature {
	...
	public static void main(String[] args) {
		String filePath = "/mars/themis/jmars/site/nomenclature/";
		String logPath = filePath + "output.txt";
		File logFile = new File(logPath);
		String baseUrl = "https://planetarynames.wr.usgs.gov/shapefiles/";
		String[] bodyList = new String[]{"MARS","MOON","MERCURY","VENUS","IO","EUROPA","GANYMEDE","CALLISTO",
				"MIMAS","ENCELADUS","TETHYS","DIONE","RHEA","TITAN","IAPETUS","PHOEBE","VESTA","ARIEL","MIRANDA","OBERON",
				"UMBRIEL","TITANIA","TRITON", "EROS", "PHOBOS", "DEIMOS", "CERES", "ITOKAWA", "PLUTO", "CHARON"};
...
//					StringBuffer log2 = new StringBuffer();
					//set the kmz name
					String bodyFileName = body + "_nomenclature.kmz";
//					System.out.println("body: "+bodyFileName+"\n");
					...
					//download and unzip
					//Backing out the changes for Proxy support. This class is used independently from JMARS. It is compiled and 
					//run on the cron server. The fewer dependencies it has, the easier it is to maintain. It does not need to run
					//behind a proxy. 
					FileUtils.copyURLToFile(fileToDownload, downloadedFile);               // TODO (PW) Use JmarsHttpRequest()
//					JmarsHttpRequest httpRequest = new JmarsHttpRequest(fileToDownload.toString(), HttpRequestType.GET);
//					boolean successful = httpRequest.send();
//					if (successful) {
//					    InputStream dowloadFileInputStream = httpRequest.getResponseAsStream();
//					    FileOutputStream outputStream = new FileOutputStream(downloadedFile);
//					    int bytesRead = 0;
//					    byte[] bytes = new byte[1024];
//					    while (( bytesRead = dowloadFileInputStream.read(bytes)) != -1 ) {
//					        outputStream.write(bytes, 0, bytesRead);
//					    }
//					    httpRequest.close();
//					    outputStream.close();
//					} else {
//					    System.out.println("unable to download nomenclature file "+baseUrl+bodyFileName);
//					}
					
					ZipFile zip = new ZipFile(downloadedFile);
					ZipEntry zipEnt = new ZipEntry("doc.kml");
